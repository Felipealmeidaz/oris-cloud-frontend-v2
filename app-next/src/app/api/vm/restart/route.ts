...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!validation.success) {
      logger.warn('Dados inválidos no restart VM', { errors: validation.error.format(), userId: user.id });
    ...
    if (!disk.isActive) {
      logger.warn('Tentativa de reiniciar VM com disco inativo', { diskName, userId: user.id });
      return NextResponse.json(
    ...
    if (new Date(disk.validUntil) < new Date()) {
      logger.warn('Tentativa de reiniciar VM com disco expirado', { diskName, userId: user.id, validUntil: disk.validUntil });
    ...
    logger.info('Iniciando processo de restart VM', { diskName, userId: user.id });

    // Verificar se o disco está anexado a uma VM
    const vmInfo = await awsService.getVMInfoByDiskName(diskName);

    if (!vmInfo) {
      logger.warn('Nenhuma VM encontrada para restart', { diskName, userId: user.id });
      return NextResponse.json(
    ...
    // Reiniciar a VM
    await awsService.restartVirtualMachine(vmInfo.awsVmName);

    logger.info('VM reiniciada com sucesso', { diskName, userId: user.id, vmName: vmInfo.awsVmName });

    return NextResponse.json({
      success: true,
      message: 'VM reiniciada com sucesso',
      vm: {
    ...
    logger.error('Erro ao reiniciar VM', { error: error.message, stack: error.stack });
    
    return NextResponse.json(
      { error: 'Erro ao reiniciar VM. Tente novamente.' },
      { status: 500 }
    );
  }
}
